...
적용 예
 - 한 쪽 테이블의 열 만 결과 집합으로 요구(일명 Semi Join)
...
*******************************************************************************
파생테이블(Derived Table, Inline View)
...
용도
 - 고급 쿼리 핵심 구문(기능)
  ; 데이터 전-처리(가공) 등 복잡한 비즈니스 논리 구현
  ; 기본 구문 한계 해결
  ; 쿼리 튜닝

작성 규칙
 - ORDER BY 절은 TOP/OFFSET/FOR XML 등 필요
...
1) 기본 CTE
2) CTE + UPDATE
3) 다중 CTE(첨언-Nested vs. Multi 분리 표현 필요)
 - 중첩(Nested) CTE - Paging 쿼리
 - 다중(Multi) CTE
4) CTE 구문 장점(Self Join) - 전월 매출 비교
5) 매 5th주문 마다 할인
...
*******************************************************************************
기능
 - 또 다른 테이블 표현식(2가지 구문 형태)
  1) 파생 테이블 + 상관 서브쿼리의 조합
    ; 일반 JOIN 형태도 가능
  2) 테이블 값 함수(Table-valued Function) 호출
    ; 매개 변수 전달
동작
 - (논리적) 왼쪽(외부) 입력의 각 행별로 테이블 표현식 반복 호출
   ; SELECT 결과를 다시 왼쪽 입력에 결합(JOIN)
종류
 - CROSS APPLY(INNER JOIN에 해당)
 - OUTER APPLY(LEFT OUTER JOIN에 해당)
  ; 일반 JOIN 형태로 사용할 경우 결과 집합이 다를 수 있음.
...
- 테이블 반환 함수 호출
 1) inline 테이블 반환 함수
 2) 다중 문 테이블 반환 함수(조직 차트) - BOL 참조
- xml 데이터 처리
- 그룹별 상위(N) 목록 - 과제(동률처리 감안)
*/
/*
-------------------------------------------------------------------------------
...
예제
 ...
 - 연속 구간 번호 찾기
 - Paging 쿼리
 - 서브쿼리 이용한 행 번호 구현(Join/SubQuery에서 다룸)

RANK, DENSE_RANK 순위

기능
 - 행 순위
 - 중복 값 발생 (ex. 1,2,2,?) 후 순위 결정
  ; RANK는 + N, 비 연속 순위
  ; DENSE_RANK는 연속 순위
 - 예제
  1) 순위
...
NTILE - 그룹 순번
기능
 - Tile(Group, Bucket) 별 순번 달기
 - Tile 별 행 수 = 전체 행 수 / Tile 수
   ; 남은 행은 Tile 순으로 1개씩 배정
...
기능
 - 드디어 Paging 쿼리 문법 - ROW_NUMBER()는 가라~~
 - OFFSET  -N개 행 (SKIP) 후
 - FETCH   -N개 행 (Window or page) 검색 (Take)

특징
 - ORDER BY 절 부속
 - 또 다른 Offset 함수 (분석 함수 범주)
 - ANSI SQK - 따라서 권장
 - LINQ to SQL 같은 API에서도 호출

예제
 - Paging 쿼리(Live 데모)
 - 그룹별 /Vth 행 검색 (파생 테이블 과제)
